...
module ImageFlags = ImageFlags
module LineCap = LineCap
module LineJoin = LineJoin
module Matrix = Matrix
module Winding = Winding
module FloatOps = FloatOps

module FontContext = Fontstash
...
module Image = struct
    ...
    let from_buffer t ~(data : data) ~width ~height ~flags : image option =
        Gles.create_texture t.impl ~type_:`RGBA ~flags ~w:width ~h:height ~data
    ;;
    ...
    let from_color t ~data ~width ~height ~flags =
        ...
        let img = Buffer.UByte.create (width*height*4) in
        for i=0 to len-1 do
            let color : Color.t = data.(i) in
            Buffer.UByte.set img (i*4+0) (int_of_float (color.r*.255.));
            Buffer.UByte.set img (i*4+1) (int_of_float (color.g*.255.));
            Buffer.UByte.set img (i*4+2) (int_of_float (color.b*.255.));
            Buffer.UByte.set img (i*4+3) (int_of_float (color.a*.255.));
        done;
        from_buffer t ~data:img ~flags ~width ~height
    ;;

    let update_image t ~image ~data =
        let w, h = Gles.get_texture_size t.impl ~image in
        Gles.update_texture t.impl ~image ~x:0 ~y:0 ~w ~h ~data
    ;;

    let size t image =
        Gles.get_texture_size t.impl ~image
    ;;
...
let begin_frame t ~width ~height ~device_ratio =
    ...
    Gles.viewport t.impl ~width ~height ~dpi:device_ratio;
...
let stroke_prepare (t : t) (state : State.t) =
    ...
    if Gles.edge_antialias t.impl && state.shape_anti_alias then (
...
let end_frame t =
    Gles.flush t.impl t.cache.verts;
...
let set_fill_paint t ~paint =
    let state = get_state t in
    state.fill <- Paint.P.copy paint;
    Paint.P.multiply state.fill state.xform;
;;
...
let set_stroke_paint t ~paint =
    let state = get_state t in
    state.stroke <- Paint.P.copy paint;
    Paint.P.multiply state.stroke state.xform;
;;
...
let flush_text_texture t =
    ...
    | Some (x0, y0, x1, y1) ->
        ...
        if image <> 0 then (
            ...
            and w = x1 - x0
            and h = y1 - y0 in
            Gles.update_texture t.impl ~image ~x ~y ~w ~h ~data |> ignore
        )
;;
...
let create ~flags gl_impl font_impl =
    let t = {
        impl = gl_impl;
            (*Gles.create ~flags arg |> opt_exn;*)
        ...
        fs = (*FontContext.create();*) font_impl;
    ...
    let value = Gles.create_texture t.impl ~type_:`Alpha ~w ~h ~flags:ImageFlags.no_flags ~data in
